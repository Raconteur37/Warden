...
async def removegems(ctx, *, state):
    ...
    currentAmount = int(getInstanceInTuple(testTable, 2))
...
@client.command()
@commands.has_permissions(manage_messages=True)
async def gems(ctx): #clean to give gems amount
    id = ctx.author.id
    c.execute('SELECT * FROM warden WHERE id IN (SELECT id FROM warden WHERE id = ?)', (id,))
    testId = c.fetchall()
    amount = getInstanceInTuple(testId, 2)
...
async def chatchecker(ctx):
    ...
    global chatChannels
    for z in chatChannels: # May not need to loop into every msg
        global msgStringCache
        ...
        for a in messages:
            if (a.author.bot != True): 
                ...
                for x in msgStringCache:
                    ...
                    # Reset after each instance and refactor the algorithm
...
def rewardGems(id, msgs, name): #Doesnt add or round properly 
        gemsAdd = 0
        b = 0
        msgCount = msgs
        while b < 1:
...
def gemsEdit(victim:int, amount:int, add:bool):
    if add == True:
        with conn:
            c.execute(
                'UPDATE warden SET gems = gems + ? WHERE id=?', 
